...
→ [Getting Started](getting-started) — install and run your first test in 5 minutes

→ [DSL Reference](dsl-reference) — `@scenario`, `VUser`, `LoadClient`, thresholds

→ [CLI Reference](cli-reference) — all flags and commands

→ [Distributed Mode](distributed) — scale across machines

→ [Benchmark](benchmark) — how LoadPilot compares to Locust
